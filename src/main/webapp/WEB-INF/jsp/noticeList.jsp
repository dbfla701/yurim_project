...
<body>
	...
	<ul class="nav navbar-nav navbar-right">
		...
		<select id="order"
			name="order" class="form-control" onchange="if(this.value) location.href=(this.value);">
				...
				<option value="${pageContext.request.contextPath}/noticeList.do?num=1&row=regdate">최신순</option>
				<option value="${pageContext.request.contextPath}/noticeList.do?num=1&row=title">글자순</option>
				<option value="${pageContext.request.contextPath}/noticeList.do?num=1&row=renum">조회순</option>
		</select></li>
	</ul>
	...
	<div style="width: 1200px; text-align: right">
		...
		<button type="button" class="btn btn-default"
			onclick="location.href='admin/noticeWrite.do'">글쓰기</button>
			...
			<ul class="pagination">
				<li><c:if test="${prev}">
						<span class="paging_prev"> <c:if test="${select == 1}">
                                [ <a
									href="${pageContext.request.contextPath}/noticeList.do?num=1">이전</a>
                                ]
                            </c:if> <c:if test="${select > 1}">
                                [ <a
									href="${pageContext.request.contextPath}/noticeList.do?num=${select - 1}">이전</a>
                                ]
                            </c:if>
						</span>
					</c:if></li>
				...
				<li><c:forEach begin="${startPageNum}" end="${endPageNum}"
						...
						<span class="paging_now"> <c:if test="${select != num}">
								<a
									href="${pageContext.request.contextPath}/noticeList.do?num=${num}&row=${row}">${num}</a>
				...
				<li><c:if test="${next}">
						<span class="paging_next"> <c:if test="${select == 1}">
                                [ <a
									href="${pageContext.request.contextPath}/noticeList.do?num=1">다음</a>
                                ]
                            </c:if> <c:if test="${select > 1}">
                                [ <a
									href="${pageContext.request.contextPath}/noticeList.do?num=${select + 1}">다음</a>
                                ]
                            </c:if>
						</span>
					</c:if>
